...
#     raise KeyError("The 'name' column is missing. Check the Excel file formatting.")

# # Define a consistent vector size (equal to the number of unique names)
# unique_names = df["name"].tolist()

# # Iterate over DataFrame
# for idx, row in df.iterrows():
#     name = row["name"]
#     synonyms = eval(row["synonyms"]) if pd.notna(row["synonyms"]) else {}  # Convert JSON to dict
#     age = row["age"]

#     # Generate vector with partial match logic
#     vector = []
#     for other_name in unique_names:
#         # If the full name is found in synonyms, use the value
#         if other_name in synonyms:
#             vector.append(synonyms[other_name])
#         # If a substring of the name appears, take the max similarity
#         else:
#             max_score = max(
#                 [synonyms[key] for key in synonyms if key in other_name] or [0.1]
#             )
#             vector.append(max_score)

#     # Print the processed data
#     print(f"Processing Name: {name}")
#     print(f"Synonyms & Weights: {synonyms}")
...
required_columns = {"name", "synonyms", "vector", "age"}
...
collection_name = "employee_relationships1"
...
for index, row in df.iterrows():
    ...
    points.append(PointStruct(id=index, vector=vector, payload={"name": name, "synonyms": synonyms, "age": age}))
